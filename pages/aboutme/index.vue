...
<template>
	<div class="min-w-full">
		...
		<section class="cv-section">
			<div class="section-box mx-8 px-5">
				<div class="flex flex-row justify-center items-center flex-wrap lg:flex-nowrap">
					...
					<div class="border-t-2 lg:border-t-0 border-solid border-foreground-tertiary p-2 lg:pr-0">
						<div class="flex flex-row lg:flex-col flex-wrap justify-center lg:justify-start">
							...
							<a href="https://github.com/CodingTil" class="mx-2 flex items-center text-foreground-primary">
								...
								<span class="ml-2 whitespace-nowra">CodingTil</span>
							</a>
						</div>
					</div>
				</div>
		...
		<!-- Projects -->
		<section class="cv-section bg-background-tertiary text-foreground-primary">
		...
		<!-- Experience -->
		<section v-if="experiences.length != 0" class="cv-section bg-background-secondary text-foreground-primary">
			<div class="section-box">
				<div class="section-title border-foreground-secondary">
					{{ $t('experience') }}
				</div>
...
export default {
	...
	computed: {
		faEnvelope() { return faEnvelope; },
		faGithubSquare() { return faGithubSquare; },
	},
	async asyncData(context) {
		const { $content, app } = context;
		const about = await $content(`${app.i18n.locale}/about`).sortBy('slug').fetch();
		const intro = about[0];
		const more = about[1];
		const projects = await $content(`${app.i18n.locale}/projects`).sortBy('slug', 'desc').fetch();
		const experiences = [];
		const teachings = await $content(`${app.i18n.locale}/teaching`).sortBy('slug', 'desc').fetch();
		const educations = await $content(`${app.i18n.locale}/education`).sortBy('slug', 'desc').fetch();
